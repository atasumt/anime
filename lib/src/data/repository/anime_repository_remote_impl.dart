...
class AnimeRepositoryRemoteImpl extends BaseRepositoryRemote implements AnimeRepositoryRemote {
  final JikanMoeApiService _animeApiService;

  AnimeRepositoryRemoteImpl(this._animeApiService);

  @override
  Future<DataStatePagination<List<AnimeData>, PaginationData>> getAnimeSearch() async {
    DataState<AnimeResponse> dataStatePaginationAnimeResponse = await getStateOf<AnimeResponse>(
  ...
  @override
  Future<DataStatePagination<List<AnimeData>, PaginationData>> getAnimeSeasonNow(int page, int limit) async {
    DataState<AnimeResponse> dataStatePaginationAnimeResponse = await getStateOf<AnimeResponse>(
  ...
  DataStatePagination<List<AnimeData>, PaginationData> _getStateForReturn(DataState<AnimeResponse> dataStateAnimeResponse) {
    if (dataStateAnimeResponse is DataStateSuccess) {
      return DataStatePaginationSuccess(dataStateAnimeResponse.data!.dataAnimeResponses!.map((dataAnimeResponse) => dataAnimeResponse.toAnimeData()).toList(),
      ...
      return DataStatePaginationError(RepositoryRemoteException('The Base Repository Remote, Data State Loading error should never occur'));
    }
  }

  @override
  Future<DataState<CharacterResponse>> getAnimeCharacters(int id) async {
    DataState<CharacterResponse> dataStatePaginationAnimeResponse = await getStateOf<CharacterResponse>(
      request: () => _animeApiService.getAnimeCharacters(id),
    );

    return dataStatePaginationAnimeResponse;
  }
}
